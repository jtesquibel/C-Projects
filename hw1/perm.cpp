...
void perm_helper(char *src_chars, char* buf, int curr)
{
  if(buf[curr] == '\0'){
    ...
    {
      char * l;
      l = strchr(buf, src_chars[i]); // hec to see if the letter has been placed in buf
      if (l == NULL) // if the letter has not been placed in buf yet
      {
        buf[curr] = src_chars[i]; // add the letter to buf
