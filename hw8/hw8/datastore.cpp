...
{
	std::cout << "add review" << std::endl;
	reviews_.push_back(r);
	map2_.find(r->prodName)->second.push_back(r);
	// map3_.find(r->userName)->second.push_back(r);
}
		...
		for (unsigned int i = 0; i < products_.size(); i++)
		{
			...
			for (it = terms.begin(); it != terms.end(); ++it)
			{
				...
				*it = convToLower(*it);
				...
				{
					// k = itt
					const char* p = strstr((*itt).c_str(), (*it).c_str());
...
std::vector<Review*> MyDataStore::getProdReviews(std::string prodName)
{
	// std::cout << "In getReviews" << std::endl;
	// if (map2_.find(prod))
	std::cout << "prod name = " << prodName << std::endl;
	std::cout << "Certain # of reviews = " << map2_.find(prodName)->second.size() << std::endl;
	return map2_.find(prodName)->second;
}

std::vector<Review*> MyDataStore::getUserReviews(std::string userName)
{
	// return map3_.find(userName)->second;
}
...
double MyDataStore::calculateAvgRating(std::string prodName)
{
	...
	int count = 0;
	std::vector<Review*> reviews = getProdReviews(prodName);
